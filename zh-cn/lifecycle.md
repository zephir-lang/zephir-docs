...
生命周期钩子注册在`config.json`。 如上图所示，有四种生命周期钩子 — `globals`， `initializers`，`destructors`，`info`。 Each of these has its own corresponding root-level setting in the configuration, and both [globals](/0.10/en/globals) and [info](/0.10/en/phpinfo) have their own chapters. 本章将介绍另外两种设置。
